...
def load_data_wrapper():
    ...
    code."""
    tr_d, tr_l, te_d, te_l = load_data()
    
    training_inputs = [np.reshape(x, (784, 1))/255 for x in tr_d]
    training_labels = [vectorized_result(y) for y in tr_l]
    
    test_inputs = [np.reshape(x, (784, 1))/255 for x in te_d]

    return training_inputs, training_labels, test_inputs, te_l
